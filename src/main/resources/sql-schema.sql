...
create table if not exists ims.orders_items(id int primary key auto_increment, order_id int not null, item_id int not null, foreign key (order_id) references orders(id), foreign key (item_id) references items(id));